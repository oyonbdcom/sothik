...
export default function DoctorManagement() {
  ...
  const handleRemoveDoctor = async (id: string) => {
    if (
      window.confirm(
        "আপনি কি নিশ্চিতভাবে এই ডাক্তারকে আপনার এরিয়া থেকে সরাতে চান?",
      )
    ) {
      try {
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-10 pt-6">
        ...
        <div className="flex bg-slate-100 p-1.5 rounded-[1.5rem] mb-6 w-full max-w-md mx-auto shadow-inner border border-slate-200">
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-[1.2rem] text-xs font-black transition-all ${
              activeTab === "all"
        ...
        <div className="flex gap-2 mb-4">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-11 pr-4 py-3.5 bg-white shadow-sm rounded-2xl outline-none border border-slate-100 text-sm font-medium focus:ring-2 ring-emerald-500/10"
            />
          </div>
          <button
        ...
        >
          ...
          >
            <option value="">
              {isAreaLoading ? "লোড হচ্ছে..." : "সব এরিয়া"}
            </option>
            {areas.map((a: any) => (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {doctors.length > 0 ? (
            doctors.map((doc: IDoctorResponse) => (
              <div
                key={doc.id}
                className="bg-white rounded-3xl p-4 border border-slate-100 shadow-sm relative group transition-all hover:border-emerald-100"
              >
