...
const MembershipCard = ({
  ...
  const doctorLink = `/doctors/${membership?.doctor?.slug}`;
  const doctorName = membership?.doctor?.user?.name || "ক্লিনিক";
        ...
        <div className="absolute -bottom-6 left-3 z-10">
          <div className="relative w-20 h-20 rounded-xl overflow-hidden border-2 border-white shadow-md bg-white">
            {membership?.doctor?.user?.image ? (
              <Image
                src={membership?.doctor?.user?.image || ""}
                alt={doctorName}
                fill
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center gap-1 mt-0.5 text-slate-500">
            ...
            <p className="text-[10px] font-medium line-clamp-1">
              {membership?.doctor?.department?.name}
            </p>
          </div>
          ...
          <div className="flex items-center justify-between mt-2 py-1 border-y border-slate-50">
            <div className="flex items-center gap-1">
              <RatingField
                value={membership?.doctor?.averageRating || 0}
                readOnly
                size={12}
              />
              <span className="text-[10px] text-slate-700 font-bold">
                {membership?.doctor?.averageRating?.toFixed(1) || "0.0"}
              </span>
            </div>
            <span className="text-[9px] text-slate-400 font-medium">
              {(membership?.doctor?.reviewsCount || 0).toLocaleString("bn")}{" "}
              রিভিউ
            </span>
          </div>
